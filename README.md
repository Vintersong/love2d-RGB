...
- **Ring-boss fight structure:** the core is **invulnerable in P1–P3** (the ring shields it) and only **exposed/damageable in P4**. Phases advance by **time** — `ringBoss.phaseDuration` seconds dodging each of P1/P2/P3, then P4 is terminal (burn the exposed core to win). Progression is time-driven, not HP-driven, because an invulnerable core can't lose HP to reach P4. In P4 the boss has its full HP pool (it took no damage earlier), so tune `BossProgression` health / `phaseDuration` for pacing.
