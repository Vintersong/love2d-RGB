...
- **Win condition (current):** the run is won by **defeating the main boss** — the ring boss, which is the final encounter (`Config.boss.ringBoss.encounterIndex = 3`, i.e. two ordinary **mini bosses** at encounters 1–2 come first, then the ring boss). `ringBoss.enabled` and `ringBoss.use_ring_boss_wincon` default **true**, so killing the exposed P4 core is the only victory; the song finishing no longer wins (the playlist just loops). Mini bosses take damage normally and are cleared to progress; only the ring-boss core kill ends the run as a win. To restore the original **track-completion** win, set both flags back to `false`. For fast iteration, set `encounterIndex = 1` (or lower `Config.bossMeter.bossInterval`).
- **Ring-boss fight structure:** the boss is damageable throughout, and its four phases are the four **HP quarters** (`ringBoss.phaseThresholds = {0.75, 0.5, 0.25}` → P1 100–75%, P2 75–50%, P3 50–25%, P4 25–0%). So damage visibly drives the fight and the boss **health bar is split into 4 segments**, one per phase, with the active phase outlined (`BossPanel`). Phase advance is **clamped** — at most one phase per step, after a `ringBoss.minPhaseDwell` (1.5s) minimum, so a burst that crosses several thresholds steps through every phase instead of skipping. Killing it (HP 0, in P4) wins. **Total damage dealt to bosses** is tracked (`BossSystem.totalDamageDealt`) and shown on the victory / full-recap screens as `BOSS DMG`. P3 laser beams span the full screen and render **behind** the boss body for a more layered look.
