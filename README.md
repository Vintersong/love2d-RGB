...
- **Ring-boss fight structure:** the boss is damageable throughout, and its four phases are the four **HP quarters** (`ringBoss.phaseThresholds = {0.75, 0.5, 0.25}` → P1 100–75%, P2 75–50%, P3 50–25%, P4 25–0%). So damage visibly drives the fight and the boss **health bar is split into 4 segments**, one per phase, with the active phase outlined (`BossPanel`). Killing it (HP 0, in P4) wins. **Total damage dealt to bosses** is tracked (`BossSystem.totalDamageDealt`) and shown on the victory / full-recap screens as `BOSS DMG`.
