...
- `src/patterns/RingBoss.lua` is the **pure** final-boss choreography/config module (dodecagonal ring + core, four phases, 6/6 telegraphed pillar curtain, interval lasers, per-phase attack generation). Live integration:
  ...
  - **Phase 2 (in progress):** when `Config.boss.ringBoss.enabled` (default **off**), `BossSystem` attaches ring phase state, advances P1→P4 by HP, fires `RingBoss.phaseAttack` from the node positions through `PatternSpawner` on `fireCadence`, and draws the ring overlay (`BossSystem:drawRing`). P4 exposes the core (only-vulnerable-then). P3 fires real laser beams via `src/combat/LaserBeam.lua` — a telegraph→active→done line segment with pure point/segment collision; `BossSystem` builds/updates/draws them and `BossCoordinator` applies beam→player damage. While ring state is attached the archetype movement/attack selection is **suppressed** and replaced by per-phase ring motion (`RingBoss.phaseVelocity`: P2 chases the player, other phases hold) so the fight reads as one reconfiguring entity. **Still pending:** pillar-telegraph rendering, and selecting the ring boss for a dedicated final encounter (today, enabling the flag makes every boss a ring boss).
