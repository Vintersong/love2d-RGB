...
- `src/patterns/RingBoss.lua` is the **pure** final-boss choreography/config module (dodecagonal ring + core, four phases, 6/6 telegraphed pillar curtain, interval lasers). Its only live integration is the **win condition**, behind `Config.boss.ringBoss.use_ring_boss_wincon` (default **off**).
