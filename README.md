...
- `src/patterns/RingBoss.lua` is the **pure** final-boss choreography/config module (dodecagonal ring + core, four phases, 6/6 telegraphed pillar curtain, interval lasers, per-phase attack generation). Live integration:
  ...
  - **Phase 2 (in progress):** when `Config.boss.ringBoss.enabled` (default **off**), `BossSystem` attaches ring phase state, advances P1→P4 by HP, fires `RingBoss.phaseAttack` from the node positions through `PatternSpawner` on `fireCadence`, and draws the ring overlay (`BossSystem:drawRing`). P4 exposes the core (only-vulnerable-then). **Still pending:** a laser/beam entity (P3 lasers are generated as `type="laser"` descriptors but skipped by the bridge for now), collapse/spread movement, telegraph rendering, and selecting the ring boss for a final encounter.
