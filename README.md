...
- **Testing the ring boss:** the run normally ends when the music track finishes (track-completion win), which cuts a boss fight short. Set `Config.debug.endlessMode = true` to suppress that win so the playlist loops/cycles and play is endless; combine with `Config.boss.ringBoss.enabled = true` (ring boss at `ringBoss.encounterIndex`, default the first boss at 100 kills) to reach and balance the four-phase fight. Death and the P4 core-kill win still end the run. Both flags default **false**.
